...
| `winmq.exe -install` | 安装为 Windows 服务**并立即启动**（自动启动 + 崩溃自动重启），**需管理员权限** |
...
打开 `winmq.exe`，点窗口上的 **「安装服务」** → 确认对话框点「是」→ 在 UAC 弹窗里确认提权即可。安装前会自动停掉窗口自己的实例（服务与窗口不能同时占用同一份数据目录和同一个端口），安装成功后会立即启动服务。信息区会实时显示服务状态（未安装 / 已停止 / 运行中），不需要时点「卸载服务」一键还原。

**方式二：命令行**（适合脚本化部署）

```powershell
# 以管理员身份执行：一条命令完成"安装 + 立即启动"
